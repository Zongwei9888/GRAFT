    ...
    def test_config_resolution_and_state_are_outside_workspace(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict(
                ...
                observe = resolve_config(workspace)
                self.assertEqual(observe.source, "observe")
                subprocess.run(
                    ["git", "init", "-q", str(workspace)], check=True
                )
                safe = resolve_config(workspace)
                self.assertEqual(safe.source, "safe-git")
                self.assertEqual(
                    [item.verifier_id for item in safe.load().verifiers],
                    ["git-diff-check"],
                )
    ...
    def test_project_commands_require_hash_trust_and_changes_revoke_it(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict(os.environ, {"GRAFT_CONFIG_HOME": str(config_home)}):
                ...
                self.assertEqual(resolve_config(workspace).source, "safe-git")
